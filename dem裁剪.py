...
shp_path = r"H:\slope\huapo_dem.shp"  # shp文件的路径， shapefile不支持中文路径
print(shp_path)
cell = 3000
...
tif_files=[r"H:\slope\2017huapo_dem.tif",r"H:\slope\2019huapo_dem.tif"]

# 生成文件总文件夹
# all_dir = r"H:\slope\bengta\\"
...
for tif_file in tif_files:
    ...
    for i in range(len(shapeRecs)):
        ...
        shp = shapeRecs[i].shape  # 获取shp文件中的每一个形状
        ...
        # proj_point=[]
        # for ii in point:
        #     proj_p=lonlat2geo(dataset,ii[0],ii[1])
        ...
        min_rect=cv2.minAreaRect(point)
        min_rect1=cv2.boxPoints(min_rect)

        len1=getDistance(min_rect1[0,1],min_rect1[0,0],min_rect1[1,1],min_rect1[1,0])
        len2=getDistance(min_rect1[2,1],min_rect1[2,0],min_rect1[1,1],min_rect1[1,0])
        ...
        if(values.size!=0):
            ...
            deg=(value_max-value_min)/length
            deg1=np.rad2deg(math.atan(deg))
            if(length<500):
                print(length,shapeRecs[i].record[0])
            # print(i,value_min,value_max,value_max-value_min, length,deg1,shapeRecs[i].record[0])
