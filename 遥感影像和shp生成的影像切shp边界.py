...
sf = shapefile.Reader(r"I:\weiyigaosu\all\huapo_own.shp")  # 读取shp文件
dataset = gdal.Open(r"I:\weiyigaosu\all\DOM\all.tif")
dataset_mask = gdal.Open(r"I:\weiyigaosu\all\huapo_own.tif")
...
output_folder = r"I:\weiyigaosu\all\clip2048_image_mask\image"
output_folder_mask = r"I:\weiyigaosu\all\clip2048_image_mask\mask"
cell = 2048
...
for i in tqdm.tqdm(range(len(shapes))):
    ...
    cv2.imwrite(os.path.join(output_folder, 'weiyigaosu_{0}_{1}.png'.format(offset_x, offset_y)), image)
    cv2.imwrite(os.path.join(output_folder_mask, 'weiyigaosu_{0}_{1}.png'.format(offset_x, offset_y)), out_bandmask)
    #
    #     j+=cell
    # i+=cell
